...
!Create output
 ...
 IF(flag_exist) THEN
  ...
  !Add bias
  CALL bhm_draw_bias((0.0,0.0),.4,0.0**2*[1,1])
  DO i3=1,fsize(3)
...
!Add small-scale wind field
 ...
 WRITE(*,*) 'construct small-scale wind field'
  ...
  !mask
  WHERE( ANY(ABS(uvEOF).GT.eps,3) )
   bhm_mask=.TRUE.
  ELSEWHERE
   bhm_mask=.FALSE.
  END WHERE 
  write(*,*) 'count mask',count(bhm_mask)/dble(size(bhm_mask))
  
 CALL bhm_cal_DB_order();
 !Standard deviation different wavelet coefficients
 ALLOCATE(sig_cS(bhm_n_DB_orders)); sig_cS=0.0
 DO i1=1,6; sig_cS(i1)=2**(1.6*i1); END DO
 !Correlation wavelet coefficients at consequetive times
 ALLOCATE(corr_cS(bhm_n_DB_orders))
 corr_cS=0.4
 
 !Renormalize small-scale field
 CALL bhm_draw_cS(corr_cS,sig_cS)
 CALL bhm_compose_uvS()
 bhm_uvS=sqrt(3.4)*bhm_uvS/&
 &SQRT( SUM(real(bhm_uvS)**2+imag(bhm_uvS)**2)/dble(2*size(bhm_uvS)) )

 !Remove outliners
 WHERE( ABS(bhm_uvS).GT.3.*SQRT(3.4) )
  bhm_uvS=bhm_uvS/ABS(bhm_uvS)*2.*SQRT(3.4)
 END WHERE
